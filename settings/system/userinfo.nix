{ config, ... }:
{
  users.users.${config.username}.openssh.authorizedKeys.keys = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIpOQirGxMfUl3F8KQxjzDZg0POSIpeNk5ayZQvugQOm li.aldric@gmail.com"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPlXUXQeII1CLwhf83/Y8fMLQ0qAai7MrthZrhElsRhF li.aldric@gmail.com#debian"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHg3soSwDR48yhLBm6KvBQliWcNbaPgd1D3QEk+Nu8EN li.aldric@gmail.com#wsl"
    "ssh-rsa  parsifal@LAPTOP-ALDRIC" # windows
    "ssh-rsa  li.aldric@gmail.com" # wsl
  ];
}
